...
def compile_pattern() -> re.Pattern:
    ...
    return re.compile(
        ...
        r"|(https://github\.com/[^/\s]+/[^/\s]+/releases/download/[^/\s]+/[^\s]+\.whl)",
        re.MULTILINE,
    )


def check_dependencies(
    lines: list, compiled_pattern: re.Pattern, path: Path, root_dir: Path
) -> bool:
    """
    ...
    for line in lines:
        if not re.search(compiled_pattern, line):
            logger.error(f"Specific dependency in {path} does not conform to the template.\n{line}")
            return False
    return True
